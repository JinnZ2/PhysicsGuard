...
# ---------------------------------------------------------------------
#
# A trait waveform that takes BOTH the immediate-state axes AND
# environment-history axes, and produces a different surface for
# each environment slice.
# ---------------------------------------------------------------------
...
class EnvironmentModulatedWaveform:
    ...
    name: str
    group: str
    base_axes: tuple                                                # immediate-state axes
    environment_axes: tuple                                         # developmental axes
    measurement_unit: str
    base_evaluator: Callable[[dict], tuple]
    environment_modulator: Callable[[dict, tuple], tuple]
    notes: str = ""
...
def to_trait_waveform(em: EnvironmentModulatedWaveform) -> TraitWaveform:
    ...
    def evaluator(state: dict) -> tuple:
...
def _generic_environment_modulator(env: dict, base: tuple) -> tuple:
    """
...
def _strength_base_male(state: dict) -> tuple:
...
def _strength_base_female(state: dict) -> tuple:
    duration = state["duration_s"]
    load = state["load_normalized"]
    cycle_phase = state["circadian_phase"]
...
def compare_within_vs_across_environment(
    ...
    """
    env_a_clean = {k: v for k, v in env_a.items() if not k.startswith("_")}
    env_b_clean = {k: v for k, v in env_b.items() if not k.startswith("_")}
    state_a = {**base_state, **env_a_clean}
    state_b = {**base_state, **env_b_clean}
...
def _selftest():
    ...
    weird_clean = {k: v for k, v in WEIRD_SEDENTARY_PROFILE.items()
    ...
    result = compare_within_vs_across_environment(
        surface,
        env_a=WEIRD_SEDENTARY_PROFILE,
        env_b=TRADITIONAL_HIGH_LOAD_PROFILE,
        base_state=base,
    )
    print(f"    within {WEIRD_SEDENTARY_PROFILE['_label']} gender gap:      "
          f"{result['within_env_a_gender_gap']:.3f}")
    print(f"    within {TRADITIONAL_HIGH_LOAD_PROFILE['_label']} gender gap: "
