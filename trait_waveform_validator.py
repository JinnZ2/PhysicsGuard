...
class Axis:
    ...
    def validate(self, value) -> None:
        numeric_range = (
            isinstance(self.domain, tuple)
            and len(self.domain) == 2
...
class TraitSurface:
    ...
    """
    trait: str
    waveforms: dict                         # group_name -> TraitWaveform
    description: str = ""
    ...
    def evaluate_all(self, **state) -> dict:
    ...
    def map_overlap_region(
        ...
        if sign_flip:
            advisory = "scalar comparison is INVALID - direction reverses across phase"
        elif (n_overlap / n) >= 0.2:
            advisory = "scalar comparison is INVALID - distributions overlap on most of phase space"
        else:
            advisory = "narrow claim may be defensible if all axes are specified"
...
class GroupComparisonClaim:
    """Structured representation of 'group A is more X than group B'."""
    trait: str
    group_a: str
    group_b: str
...
class ClaimValidator:
    ...
    def validate(self, claim: GroupComparisonClaim) -> dict:
        ...
        return {
            "verdict": (
                "supported_at_phase_point" if claim_supported
                else "not_supported_at_phase_point"
            ),
            "comparison": comp,
...
def _strength_male(state: dict) -> tuple:
...
def _strength_female(state: dict) -> tuple:
