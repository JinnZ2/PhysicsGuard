...
def _spatial_rotation_male(s: dict) -> tuple:
    base = 1.05
...
def _spatial_rotation_female(s: dict) -> tuple:
    base = 0.95
...
def _verbal_fluency_male(s: dict) -> tuple:
    base = 0.95
    base *= _circadian_alertness(s["circadian_phase"])
    base *= _sleep_decrement(s["sleep_debt_h"])
    base *= _glucose_factor(s["glucose_state"])
...
def _verbal_fluency_female(s: dict) -> tuple:
    base = 1.05
    cycle = s["endocrine_phase"]
...
def _risk_assessment_male(s: dict) -> tuple:
    """
...
def _risk_assessment_female(s: dict) -> tuple:
    base = 1.0
    cycle = s["endocrine_phase"]
...
def _pattern_integration_male(s: dict) -> tuple:
    base = 1.0
    base *= _circadian_alertness(s["circadian_phase"])
    base *= _sleep_decrement(s["sleep_debt_h"])
    base *= _glucose_factor(s["glucose_state"])
...
def _pattern_integration_female(s: dict) -> tuple:
    base = 1.0
    cycle = s["endocrine_phase"]
...
def _sustained_attention_male(s: dict) -> tuple:
    base = 1.0
    base *= _circadian_alertness(s["circadian_phase"])
    base *= _sleep_decrement(s["sleep_debt_h"])
    base *= _glucose_factor(s["glucose_state"])
...
def _sustained_attention_female(s: dict) -> tuple:
    base = 1.0
    base *= _circadian_alertness(s["circadian_phase"])
    base *= _sleep_decrement(s["sleep_debt_h"])
    base *= _glucose_factor(s["glucose_state"])
...
def _working_memory_male(s: dict) -> tuple:
    base = 1.0
    base *= _circadian_alertness(s["circadian_phase"])
    base *= _sleep_decrement(s["sleep_debt_h"])
    base *= _glucose_factor(s["glucose_state"])
...
def _working_memory_female(s: dict) -> tuple:
    base = 1.0
    cycle = s["endocrine_phase"]
...
def _selftest():
    ...
    validator = ClaimValidator()
    register_all_cognition_surfaces(validator)
    PhysicsGuardAdapter(validator)  # smoke-test adapter wiring
    ...
    # CASE D: same task, sleep-deprived stressed afternoon
